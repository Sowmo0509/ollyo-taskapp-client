...
const TaskCard = ({ task, onTaskDeleted, status, onTaskStatusChange }: TaskCardProps) => {
  ...
  return (
    <>
      <div 
        ref={cardRef} 
        className={`bg-white p-4 shadow-lg shadow-zinc-300/40 rounded-lg items-start flex flex-col gap-2 ${isDragging ? "opacity-50" : "opacity-100"} ${isEditing ? "cursor-default" : "cursor-move"}`}
      >
        {isEditing ? (
          // Edit mode
          <>
            <div className="w-full">
              ...
              <input 
                ...
                className={`w-full rounded-md border ${errors.name ? 'border-red-500' : 'border-gray-300'} px-3 py-2 text-sm focus:outline-none focus:ring-1 ${errors.name ? 'focus:ring-red-500' : 'focus:ring-indigo-500'}`} 
              />
            ...
            <div className="w-full">
              ...
              <textarea 
                ...
                onChange={handleChange} 
                rows={2} 
                className={`w-full rounded-md border ${errors.description ? 'border-red-500' : 'border-gray-300'} px-3 py-2 text-sm focus:outline-none focus:ring-1 ${errors.description ? 'focus:ring-red-500' : 'focus:ring-indigo-500'}`} 
              />
            ...
            <div className="w-full">
              ...
              <input 
                ...
                onChange={handleChange} 
                className={`w-full rounded-md border ${errors.due_date ? 'border-red-500' : 'border-gray-300'} px-3 py-2 text-sm focus:outline-none focus:ring-1 ${errors.due_date ? 'focus:ring-red-500' : 'focus:ring-indigo-500'}`} 
              />
            ...
            <div className="flex justify-end gap-x-2 mt-2 w-full">
              <button 
                ...
                className="flex items-center gap-x-1 px-2 py-1 text-xs font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-md"
              >
                <IconX size={14} />
                Cancel
              </button>
              <button 
                ...
                className={`flex items-center gap-x-1 px-2 py-1 text-xs font-medium text-white rounded-md ${isSaving || Object.values(errors).some(error => error !== "") ? 'bg-indigo-400 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-700'}`}
              >
                <IconCheck size={14} />
